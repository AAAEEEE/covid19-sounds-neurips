...
def extract_opensmile(sample):
    ...
    cmd = 'SMILExtract -C opensmile-3.0-win-x64/config/is09-13/IS13_ComParE.conf -I "' + savepath + name + '_normalized.wav"' + ' -O ' + '"' + savepath+ name+'.ex6373_n.txt"'
    os.system(cmd)
    cmd = 'SMILExtract -C opensmile-3.0-win-x64/config/is09-13/IS09_emotion.conf -I "' + savepath + name + '_normalized.wav"' + ' -O ' + '"' + savepath+ name+'.ex384_n.txt"'
    os.system(cmd)
    print(cmd)
