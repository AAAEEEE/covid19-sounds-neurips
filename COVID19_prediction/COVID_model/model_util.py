...
def spec_augment(spec: np.ndarray,
                 ...
                 time_masking_max_percentage=0.2):
...
def load_data(data_path, is_aug):
    ...
    for uid in get_resort(data['train_covid_id']):
        for temp in data['train_covid_id'][uid]:
            train_task.append({'breath':temp['breath'],'cough':temp['cough'], 'voice':temp['voice'], 'label':[0, 1]})
            covidcnt += 1
    for uid in get_resort(data['train_noncovid_id']):
        for temp in data['train_noncovid_id'][uid]:
            train_task.append({'breath':temp['breath'],'cough':temp['cough'], 'voice':temp['voice'], 'label':[1, 0]})
            noncvcnt += 1
    ...
    if is_aug:  #only works for train
        for i in range(total):
            temp = train_task[i]
            for type in ['noise','pitchspeed']:
                train_task.append({'breath':get_aug(temp['breath'],type),'cough':get_aug(temp['cough'],type), 'voice':get_aug(temp['voice'],type), 'sym':temp['sym'], 'label':temp['label']})
    '''
    ...
    for uid in get_resort(data['vad_covid_id']):
        for temp in data['vad_covid_id'][uid]:
            vad_task.append({'breath':temp['breath'],'cough':temp['cough'], 'voice':temp['voice'], 'label':[0, 1]})
            covidcnt += 1
    for uid in get_resort(data['vad_noncovid_id']):
        for temp in data['vad_noncovid_id'][uid]:
            vad_task.append({'breath':temp['breath'],'cough':temp['cough'], 'voice':temp['voice'], 'label':[1, 0]})
            noncvcnt += 1
    print('covid:',covidcnt, 'non-covid:', noncvcnt )
    ...
    for uid in get_resort(data['test_covid_id']):
        for temp in data['test_covid_id'][uid]:
            test_task.append({'breath':temp['breath'],'cough':temp['cough'], 'voice':temp['voice'], 'label':[0, 1]})
            covidcnt += 1
    for uid in get_resort(data['test_noncovid_id']):
        for temp in data['test_noncovid_id'][uid]:
            test_task.append({'breath':temp['breath'],'cough':temp['cough'], 'voice':temp['voice'], 'label':[1, 0]})
            noncvcnt += 1
    print('covid:',covidcnt, 'non-covid:', noncvcnt)
...
def get_CI(data, AUC, Sen, Spe):
    ...
    print(str(AUC.round(2)) + '(' + str(q_0.round(2)) + '-' + str(q_1.round(2)) + ')'
