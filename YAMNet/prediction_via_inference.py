...
# 	print(yamnetData[i,0])
	...
	if ("cough" in yamnetData[i,0]) and \
	   ("Cough" in yamnetData[i, 1] or \
 		"Cough" in yamnetData[i, 2] or \
		"Cough" in yamnetData[i, 3] or \
		"Cough" in yamnetData[i, 4] or \
		"Cough" in yamnetData[i, 5]):
		prediction_list.append(['c'])
	elif ("breath" in yamnetData[i,0]) and \
	   ("Breathing" in yamnetData[i, 1] or \
 		"Breathing" in yamnetData[i, 2] or \
		"Breathing" in yamnetData[i, 3] or \
		"Breathing" in yamnetData[i, 4] or \
		"Breathing" in yamnetData[i, 5]):
		prediction_list.append(['b'])
	elif "Speech" in yamnetData[i, 1] and \
	    float(yamnetData[i,1].split(':')[1][:-1])>=0.4 and \
		("read" in yamnetData[i,0] or "voice" in yamnetData[i,0]):
...
print (data.shape)
np.savetxt(outputFile, data,fmt='%s', delimiter=";", header='filename;p1;p2;p3;p4;p5;label;firstProb;firstCatogory')
