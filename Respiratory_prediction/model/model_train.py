...
from vggish_slim import load_vggish_slim_checkpoint
...
parser.add_argument("--early_stop", type=str, default=params.EARLY_STOP, help="The indicator on validation set to stop training.")
...
parser.add_argument("--trained_layers", type=int, default=params.TRAINED_LAYERS, help="The number Vgg layers to be fine tuned.")
...
audio_ckpt_dir = os.path.join(params.AUDIO_CHECKPOINT_DIR,FLAGS['train_name'])#./data/train/ name_modality: name, with/out feature, modality: B, C, V, BCV
name_pre = FLAGS['modality'] + '_' + 'Dp' + str(FLAGS['dropout_rate']) + '_' + 'U' + str(FLAGS['num_units']) + '_' + 'R' + str(FLAGS['rnn_units']) 
...
def _add_triaining_graph():
    ...
    with tf.Graph().as_default() as graph:
        logits, logits_sym = define_audio_slim(modality=FLAGS['modality'],reg_l2=FLAGS['reg_l2'], rnn_units=FLAGS['rnn_units'],
                                             num_units=FLAGS['num_units'],modfuse = FLAGS['modfuse'], train_vgg = FLAGS['train_vgg'])
        ...
        with tf.variable_scope('train'):
            ...
            reg_loss2 = tf.add_n([ tf.nn.l2_loss(v) * FLAGS['reg_l2'] for v in tf.trainable_variables() if 'bias' not in v.name ], name='reg_loss2')
            if FLAGS['is_sym']:
            ...
            lr1 = tf.train.exponential_decay(FLAGS['lr1'], global_step, number_decay_steps, base_of, staircase=True,name='train_lr1')
            lr2 = tf.train.exponential_decay(FLAGS['lr2'], global_step, number_decay_steps, base_of, staircase=True,name='train_lr2')
            ...
            if FLAGS['is_diff']: #use different learning rate for vgg and others
                ...
                train_op1 = tf.train.AdamOptimizer(learning_rate=lr1,epsilon=params.ADAM_EPSILON).minimize(loss, var_list=var1, global_step=global_step, name='train_op1') 
                train_op2 = tf.train.AdamOptimizer(learning_rate=lr2,epsilon=params.ADAM_EPSILON).minimize(loss, var_list=var2, global_step=global_step, name='train_op2') #fixed 'var1'
    ...
    with tf.Session(graph=_add_triaining_graph(), config=sess_config) as sess:
        ...
        for epoch in range(FLAGS['epoch']):
            ...
            for ii,sample in enumerate(train_data):  #generate training batch 
                ...
                [num_steps, lr1, lr2, logits, logitsym, loss, summaries, _, clal, regl, syml] = sess.run([global_step_tensor, lr1_tensor, lr2_tensor, logit_tensor, logitsym_tensor, loss_tensor, summary_op, train_op, cla_loss_tensor, reg_loss_tensor, sym_loss_tensor],
                        feed_dict={vgg_tensor: vggcomb,  #Mel-spetrugram
                                   ...
                                   labels_tensor: [labels]})  #groud truth
                ...
                if epoch > 0 and ii%1000 == 0:
                    ...
                    for sample in valid_data:
                        vggcomb, index, index2, labels, symptom = util.get_input(sample)
                        [logits, logitsym, loss, clal, regl] = sess.run([logit_tensor,logitsym_tensor,loss_tensor,cla_loss_tensor,reg_loss_tensor ],
                                feed_dict={vgg_tensor: vggcomb, 
                    ...
                    print('Epoch {}/{}:'.format(epoch, FLAGS['epoch']), "validation loss: %g" % val_loss, "cross-entropy loss: %g" % epoch_loss, "regulation loss: %g" % epcoh_reg_loss)
                    AUC, TPR, TNR, TPR_TNR_9 = util.get_metrics(probs_all,label_all)
                    if FLAGS['is_sym']:
                        UAR_SYM = util.get_metrics2(probs_sym_all,label_sym_all)
                    vad_auc = AUC
                    logfile.write('Epoch {}/{}: AUC:{}, TPR:{}, TNR:{}, TPR_TNR_9:{}'.format(epoch, FLAGS['epoch'], AUC, TPR, TNR, TPR_TNR_9)) 
                    logfile.write('\n')
                    
                    #checkpoint_path_epoch = os.path.join(audio_ckpt_dir, str(epoch)+ '_' + name_all + params.AUDIO_CHECKPOINT_NAME)
            ...
            print('Epoch {}/{}:'.format(epoch, FLAGS['epoch']), "train epoch loss: %g" % train_epoch_loss, "cross-entropy loss: %g" % epoch_loss, "regulation loss: %g" % epcoh_reg_loss, "symptom loss: %g" % epcoh_sym_loss)
            AUC, TPR, TNR, TPR_TNR_9 = util.get_metrics(probs_all,label_all)
            if FLAGS['is_sym']:
                UAR_SYM = util.get_metrics2(probs_sym_all,label_sym_all)
            train_auc = AUC
